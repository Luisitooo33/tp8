...
if uploaded_file:
    ...
    # Calcular precio unitario para cada fila
    df['Precio_unitario'] = df['Ingreso_total'] / df['Unidades_vendidas']

    # Calcular margen para cada fila
    ...
    sumas = df.groupby('Producto').agg({
        ...
        'Ingreso_total': 'sum',
        'Costo_total': 'sum',
        'Precio_unitario': 'sum'  # Suma de precios unitarios
    })

    # Contar registros por producto
    conteo = df.groupby('Producto').size()

    # Calcular precio promedio manualmente
    ...
    for i, row in df_grouped.iterrows():
        ...
        with col2:
            ...
            # Crear fecha completa para mejor visualización
